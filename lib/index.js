...
* @param {Number} [opts.n=1] - number of trails
...
function pdf( x, options ) {
	...
	if ( isMatrixLike( x ) ) {
		...
		return pdf5( out, x, opts.n, opts.p );
	}
	if ( isTypedArrayLike( x ) ) {
		...
		return pdf6( out, x, opts.n, opts.p );
	}
	if ( isArrayLike( x ) ) {
		// Handle deepset first...
		if ( opts.path ) {
			opts.sep = opts.sep || '.';
			return pdf4( x, opts.n, opts.p, opts.path, opts.sep );
		}
		...
		if ( opts.accessor ) {
			return pdf3( out, x, opts.n, opts.p, opts.accessor );
		}
		return pdf2( out, x, opts.n, opts.p );
	}
	return NaN;
} // end FUNCTION pdf()
